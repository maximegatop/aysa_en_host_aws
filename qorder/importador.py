...
def filesuministrogu(params,oficina,cursor,_log):
	try:
		oficina=str(oficina)
		params1=params
		cadena=[]
...
def filesuministrores(params,oficina,cursor,_log):
	try:
		oficina=str(oficina)
		params1=params
		cadena=[]
...
def updatetables(listporcionprocesos,_log,oficina,strsemana, cursor):
...
def insertDatosSuministro(_log,oficina,flag_imp,strsuministros,strsemana, cursor):
    	
	try:

		if flag_imp==1: #Si ya se realizo una importacion

			sql = """
					INSERT INTO qorder_puntodesuministro 
					...
					SELECT 
						NRO_CLIENTE, SEG_REG, GPS_LATITUD, GPS_LONGITUD, ACCESO_FINCA, '', ACCESO_PM, '', 1, DIRECCION, NRO_PUERTA, PISO, DUPLICADOR, LOCALIDAD, '', '', '', CODPOST, '', 
						CONCAT(COD_MARCA, NRO_APARTO), NRO_CLIENTE, CONCAT(COD_UNICOM, RUTA, ITINERARIO), NULL, NULL, 1, SECUENCIA, NULL, 0 
					FROM qorder_suministros 
					WHERE COD_UNICOM = '{}' AND ANIO = '{}' AND SEG_REG IN ({})
					ON DUPLICATE KEY UPDATE 
						gps_latitud = IF(
							VALUES(gps_latitud) IS NOT NULL AND VALUES(gps_latitud) != '' AND FLOOR(VALUES(gps_latitud)) != 0, 
							VALUES(gps_latitud), 
							qorder_puntodesuministro.gps_latitud
						),
						gps_longitud = IF(
							VALUES(gps_longitud) IS NOT NULL AND VALUES(gps_longitud) != '' AND FLOOR(VALUES(gps_longitud)) != 0, 
							VALUES(gps_longitud), 
							qorder_puntodesuministro.gps_longitud
						),
						num_contrato = VALUES(num_contrato),
						ref_finca = VALUES(ref_finca),
						ref_direccion = VALUES(ref_direccion),
						ref_suministro = VALUES(ref_suministro),
						nif = VALUES(nif),
						estado_suministro = VALUES(estado_suministro),
						calle = VALUES(calle),
						numero_puerta = VALUES(numero_puerta),
						piso = VALUES(piso),
						duplicador = VALUES(duplicador),
						localidad = VALUES(localidad),
						municipio = VALUES(municipio),
						barrio = VALUES(barrio),
						departamento = VALUES(departamento),
						codigo_postal = VALUES(codigo_postal),
						hashdata = VALUES(hashdata),
						aparato_id = VALUES(aparato_id),
						cliente_id = VALUES(cliente_id),
						rutasum_id = VALUES(rutasum_id),
						tarifa_id = VALUES(tarifa_id),
						tipo_asociacion_id = VALUES(tipo_asociacion_id),
						tipo_servicio_id = VALUES(tipo_servicio_id),
						secuencia_teorica = VALUES(secuencia_teorica),
						fecha_actualizacion_secuencia = VALUES(fecha_actualizacion_secuencia),
						secuencia_anterior = VALUES(secuencia_anterior)
					""".format(oficina, strsemana, strsuministros)
		...
		else:  #Si se realiza la importacion por primera vez
...
def insertAparatosSuministro(_log,oficina,flag_imp,strsuministros,strsemana, cursor):
	...
	try:
		...
		if flag_imp==1:  #Si ya se realizo una importacion
			sql = "INSERT INTO qorder_aparato\
				...
				select concat(COD_MARCA,NRO_APARTO),NRO_APARTO,RUEDAS,'',null,null,null,'','',0,COD_MARCA,null,1,1,1 from qorder_suministros where ANIO='{}' and COD_UNICOM='{}' and SEG_REG in ({})\
				ON DUPLICATE KEY UPDATE num_serie=VALUES(num_serie),num_ruedas=VALUES(num_ruedas),estado_aparato=VALUES(estado_aparato),fecha_fabricacion=VALUES(fecha_fabricacion),fecha_instalacion=VALUES(fecha_instalacion),fecha_proxima_calibracion=VALUES(fecha_proxima_calibracion),diametro=VALUES(diametro),presion=VALUES(presion),coef_perdida=VALUES(coef_perdida)".format(
				strsemana, oficina,strsuministros)
...
def insertAparatoConsumo(_log,oficina,flag_imp,strsuministros,strsemana, cursor):
    	
	try:

		if flag_imp==1: #Si ya se realizo la importacion
			sql = "INSERT INTO qorder_consumo (consumo, constante, lectura_anterior, consumo_anterior, fecha_lectura_anterior, tope_lectura_maxima, tope_lectura_minima, aparato_id, tipo_consumo_id) \
				select concat(COD_MARCA,NRO_APARTO,'CO011'),1,LECTURA_ANTERIOR,CONSUMO1,CASE WHEN FECHA_LECTURA1 = '' THEN NULL ELSE FECHA_LECTURA1 END,LECTURA_MAXIMA,LECTURA_MINIMA,concat(COD_MARCA,NRO_APARTO),'CO011' " \
				"from qorder_suministros where ANIO='{}' and COD_UNICOM='{}' and SEG_REG in ({})\
				ON DUPLICATE KEY UPDATE " \
				"constante=VALUES(constante)," \
				"lectura_anterior=VALUES(lectura_anterior)," \
				"consumo_anterior=VALUES(consumo_anterior)," \
				"fecha_lectura_anterior=VALUES(fecha_lectura_anterior),tope_lectura_maxima=VALUES(tope_lectura_maxima),tope_lectura_minima=VALUES(tope_lectura_minima),aparato_id=VALUES(aparato_id),tipo_consumo_id=VALUES(tipo_consumo_id)".format(
				strsemana, oficina,strsuministros)
			try:
			...
			sql = "INSERT INTO qorder_consumo (consumo, constante, lectura_anterior, consumo_anterior, fecha_lectura_anterior, tope_lectura_maxima, tope_lectura_minima, aparato_id, tipo_consumo_id) \
				select concat(COD_MARCA,NRO_APARTO,'CO011'),1,LECTURA_ANTERIOR,CONSUMO1,CASE WHEN FECHA_LECTURA1 = '' THEN NULL ELSE FECHA_LECTURA1 END,LECTURA_MAXIMA,LECTURA_MINIMA,concat(COD_MARCA,NRO_APARTO),'CO011' " \
				"from qorder_suministros where ANIO='{}' and COD_UNICOM='{}'\
				ON DUPLICATE KEY UPDATE " \
				"constante=VALUES(constante)," \
				"lectura_anterior=VALUES(lectura_anterior)," \
				"consumo_anterior=VALUES(consumo_anterior)," \
...
def insertCliente(_log,oficina,flag_imp,strsuministros,strsemana, cursor):
	...
	try:
		if flag_imp==1:  #Si ya se realizo la importacion
			...
			clientes.delete()
		else:  #Si se realiza por primera vez la importacion
			sql = "INSERT INTO qorder_cliente (codigo, nombre, apellido_1, apellido_2, calle, numero_puerta, piso, duplicador, localidad, municipio, barrio, departamento, codigo_postal, estado_cliente, fecha_alta, observacion)select NRO_CLIENTE,'',NOMBRE_CLIENTE,'',DIRECCION,NRO_PUERTA,PISO,DUPLICADOR,LOCALIDAD,\
...
def insertrutasum(_log,oficina,flag_imp,strsuministros,strsemana, cursor):

	try:

		if flag_imp==1:  #Si ya se realizo una importacion
			sql = "INSERT INTO qorder_rutasum(idrutasum, rutasum, itinerario, oficina_id, Frecuencia)\
				select concat(COD_UNICOM,RUTA,ITINERARIO),RUTA,ITINERARIO,COD_UNICOM,null from qorder_suministros where COD_UNICOM='{}' and ANIO='{}' and SEG_REG in ({}) ON DUPLICATE KEY UPDATE rutasum=VALUES(rutasum),itinerario=VALUES(itinerario)".format(
				oficina, strsemana,strsuministros)
			...
			sql = "INSERT INTO qorder_rutasum(idrutasum, rutasum, itinerario, oficina_id, Frecuencia)\
				select concat(COD_UNICOM,RUTA,ITINERARIO),RUTA,ITINERARIO,COD_UNICOM,null from qorder_suministros where COD_UNICOM='{}' and ANIO='{}' ON DUPLICATE KEY UPDATE rutasum=VALUES(rutasum),itinerario=VALUES(itinerario)".format(
				oficina, strsemana)
...
def insertruta(_log,oficina,user,flag_imp,strsuministros,strsemana, cursor):
	...
	try:
		...
		if flag_imp==1:  #Si ya se realizo una importacion
			suministros = list(query_to_dicts("select distinct cod_unicom,ruta,itinerario,ciclo,anio,COUNT(*)as cant from qorder_suministros where cod_unicom='{}' and anio='{}' and SEG_REG in ({}) group by cod_unicom,ruta,itinerario,ciclo,anio ORDER BY itinerario,ruta;".format(oficina, strsemana,strsuministros)))
			for s in suministros:
				...
				args = (
				cod_unicom + ruta + itinerario.zfill(5) + ciclo + anio.zfill(4), ciclo, ruta, itinerario, '', anio, cant, 0,
				str(fer), fer, 1, 0, None, cod_unicom, cod_unicom + ruta + itinerario, None, None, None, fechaimp)

				try:
					cursor.execute(sql, args)
					log_rutas.objects.create(estado='Importación', fecha_log=datetime.now(),
												observacion='ruta generada',
												ruta_id=cod_unicom + ruta + itinerario.zfill(5) + ciclo + anio.zfill(4),
												usuario=user)
				except Exception as e:
					_log.error(e)
					raise e
			...
			suministros = list(query_to_dicts(
				"select distinct cod_unicom,ruta,itinerario,ciclo,anio,COUNT(*)as cant from qorder_suministros where cod_unicom='{}' and anio='{}' group by cod_unicom,ruta,itinerario,ciclo,anio ORDER BY itinerario,ruta;".format(oficina, strsemana)))
			for s in suministros:
				cod_unicom = ''
				ruta = ''
				itinerario = ''
...
def insertorden(_log,oficina,flag_imp,strsuministros,strsemana, cursor):
	...
	try:
		...
		if flag_imp==1:  #Si ya se realizo una importacion
			sql = "INSERT INTO qorder_ordendetrabajo(numero_orden, prioridad, estado, secuencial_registro, secuencia_teorica, orden_terreno, generada_desde_num_os, flag_asignacion_guardada, fecha_hora_asignacion, fecha_hora_importacion, fecha_hora_exportacion, fecha_hora_anulacion, fecha_hora_ult_modificacion, fecha_hora_carga, consumo_id, punto_suministro_id, ruta_id, tecnico_id, tipo_orden_id, usuario_asignacion_id)\
				...
				date, tipo_orden.id, oficina, strsemana,strsuministros)
...
def inserthistconsumo(_log,oficina,flag_imp,strsuministros,strsemana,cursor):
	string=''
	valorf = ''

	if flag_imp==1:  #Si ya se realizo una importacion
		suministros = list(query_to_dicts("select o.numero_orden as num_os,s.COD_MARCA,s.SEG_REG,"
										  "s.NRO_APARTO,s.COD_ANOMALIA1 as cod_anomalia1,"
										  "s.LECTURA1 as lectura1,s.CONSUMO1 as consumo1,"
										  "s.FECHA_LECTURA1 as fecha_lectura1,s.COD_ANOMALIA2 as cod_anomalia2,"
										  "s.LECTURA2 as lectura2,s.CONSUMO2 as consumo2,s.FECHA_LECTURA2 as fecha_lectura2,s.COD_ANOMALIA3 as cod_anomalia3,s.LECTURA3 as lectura3,s.CONSUMO3 as consumo3,s.FECHA_LECTURA3 as fecha_lectura3,s.COD_ANOMALIA4 as cod_anomalia4,s.LECTURA4 as lectura4,s.CONSUMO4 as consumo4,s.FECHA_LECTURA4 as fecha_lectura4,s.COD_ANOMALIA5 as cod_anomalia5,s.LECTURA5 as lectura5,s.CONSUMO5 as consumo5,s.FECHA_LECTURA5 as fecha_lectura5,s.COD_ANOMALIA6 as cod_anomalia6,s.LECTURA6 as lectura6,s.CONSUMO6 as consumo6,s.FECHA_LECTURA6 as fecha_lectura6\
        from qorder_suministros as s inner join qorder_ordendetrabajo as o on s.SEG_REG=o.punto_suministro_id "
										  "inner join qorder_ruta as r "
										  "on o.ruta_id=r.idruta and r.oficina_id='{}' and r.anio='{}' where s.COD_UNICOM='{}' and s.ANIO='{}' and SEG_REG in ({})".format(
			oficina, strsemana, oficina, strsemana,strsuministros)))

		for s in suministros:
			string = str('') + '|' + s['num_os'] + '|' + str(s['cod_anomalia1']) + '|' + str(s['lectura1']) + '|' + str(
				s['consumo1']) + '|' + s['fecha_lectura1'] + '|' + str(s['cod_anomalia2']) + '|' + str(
				s['lectura2']) + '|' + str(s['consumo2']) + '|' + s['fecha_lectura2'] + '|' + str(
				s['cod_anomalia3']) + '|' + str(s['lectura3']) + '|' + str(s['consumo3']) + '|' + s[
						 'fecha_lectura3'] + '|' + str(s['cod_anomalia4']) + '|' + str(s['lectura4']) + '|' + str(
				s['consumo4']) + '|' + s['fecha_lectura4'] + '|' + str(s['cod_anomalia5']) + '|' + str(
				s['lectura5']) + '|' + str(s['consumo5']) + '|' + s['fecha_lectura5'] + '|' + str(
				s['cod_anomalia6']) + '|' + str(s['lectura6']) + '|' + str(s['consumo6']) + '|' + s['fecha_lectura6']
        ...
        from qorder_suministros as s inner join qorder_ordendetrabajo as o on s.SEG_REG=o.punto_suministro_id "
										  "inner join qorder_ruta as r "
										  "on o.ruta_id=r.idruta and r.oficina_id='{}' and r.anio='{}' where s.COD_UNICOM='{}' and s.ANIO='{}'".format(
			oficina, strsemana, oficina, strsemana)))
...
def insertarsuministros(strsemana,oficina, cursor):
	try:
		sql = 'INSERT INTO qorder_suministros (COD_UNICOM, RUTA, ITINERARIO, CICLO, ANIO, SEG_REG, DIVISION, SECUENCIA, LOCALIDAD, NOMBRE_CLIENTE, DIRECCION, NRO_PUERTA, PISO, DUPLICADOR, NRO_APARTO, COD_MARCA, RUEDAS, MULTIPLICADOR, LECTURA_ANTERIOR,\
				LECTURA_MINIMA, LECTURA_MAXIMA, DESC_CONSUMO, ACCESO_FINCA, ACCESO_PM, ESTADO_LECT, ESTADO_SUM, ESTADO_ACT, COD_TARIFA, LECTURA_ACTUAL, TIP_CONSUMO, FH_LECTURA, CONSUMO, CANT_LECT_FORZADA, NRO_LECTURISTA, COD_COLECTOR, COD_ANOMALIA_HH_1, NOTAS1,\
				COD_ANOMALIA_HH_2, NOTAS2, COD_ANOMALIA_HH_3, NOTAS3, SECUENCIA_REAL, NRO_CLIENTE, COMPLEMENTO, CONSUMO_ESTIMADO, COD_ANOMALIA1, LECTURA1, CONSUMO1, FECHA_LECTURA1, COD_ANOMALIA2, LECTURA2, CONSUMO2, FECHA_LECTURA2, COD_ANOMALIA3, LECTURA3, CONSUMO3,\
				...
				CONSUMO5, FECHA_LECTURA5, COD_ANOMALIA6, LECTURA6, CONSUMO6, FECHA_LECTURA6, COD_RECAMBIO, LEC_MEDIDOR_RETIRADO, CONSUMO_ULTIMO_RECAMBIO, FECHA_RECAMBIO, ACUM_CONS_VARIOS_RECAMBIOS, ANIO_INSTALACION, COD_TIPO_CONEXION, TIPO_MEDIDOR, COD_DIAMETRO, CONSULTO_HISTORICOS,\
				COD_ESTADO_CONEXION, NRO_LECTURISTA_AUDITORIA, COD_UNICOM_LECT_AUDIT, COD_COLECTOR_AUDITORIA, COD_UNICOM_COLEC_AUDIT, FH_LECTURA_AUDIT, LECTURA_ACTUAL_AUDIT, COD_ANOMALIA_HH_1_AUDIT, NOTAS1_AUDIT, GPS_LATITUD, GPS_LONGITUD, PORCION_ORIGINAL, SEC_ORIGINAL, UNIDAD_ORIGINAL,\
				CODPOST, DCSM, DISTRITO, CIRC, SECCION, MANZANA, ORDENADO_POR, SEC_RELACIONADA, LAT, LON FROM qorder_suministros_res where COD_UNICOM={} and ANIO={}'.format(
			oficina,strsemana)
		...
		sql = 'INSERT INTO qorder_suministros (COD_UNICOM, RUTA, ITINERARIO, CICLO, ANIO, SEG_REG, DIVISION, SECUENCIA, LOCALIDAD, NOMBRE_CLIENTE, DIRECCION, NRO_PUERTA, PISO, DUPLICADOR, NRO_APARTO, COD_MARCA, RUEDAS, MULTIPLICADOR, LECTURA_ANTERIOR, LECTURA_MINIMA, LECTURA_MAXIMA, DESC_CONSUMO, ACCESO_FINCA, ACCESO_PM, ESTADO_LECT, ESTADO_SUM, ESTADO_ACT, COD_TARIFA, \
					...
					CONSUMO, CANT_LECT_FORZADA, NRO_LECTURISTA, COD_COLECTOR, COD_ANOMALIA_HH_1, NOTAS1, COD_ANOMALIA_HH_2, NOTAS2, COD_ANOMALIA_HH_3, NOTAS3, SECUENCIA_REAL, NRO_CLIENTE, COMPLEMENTO, CONSUMO_ESTIMADO, COD_ANOMALIA1, LECTURA1, CONSUMO1, FECHA_LECTURA1, COD_ANOMALIA2, LECTURA2, CONSUMO2, FECHA_LECTURA2, COD_ANOMALIA3, LECTURA3, CONSUMO3, FECHA_LECTURA3, COD_ANOMALIA4, LECTURA4,\
					CONSUMO4, FECHA_LECTURA4, COD_ANOMALIA5, LECTURA5, CONSUMO5, FECHA_LECTURA5, COD_ANOMALIA6, LECTURA6, CONSUMO6, FECHA_LECTURA6, COD_RECAMBIO, LEC_MEDIDOR_RETIRADO, CONSUMO_ULTIMO_RECAMBIO, FECHA_RECAMBIO, ACUM_CONS_VARIOS_RECAMBIOS, ANIO_INSTALACION, COD_TIPO_CONEXION, TIPO_MEDIDOR, COD_DIAMETRO, CONSULTO_HISTORICOS, COD_ESTADO_CONEXION, NRO_LECTURISTA_AUDITORIA, COD_UNICOM_LECT_AUDIT,\
					COD_COLECTOR_AUDITORIA, COD_UNICOM_COLEC_AUDIT, FH_LECTURA_AUDIT, LECTURA_ACTUAL_AUDIT, COD_ANOMALIA_HH_1_AUDIT, NOTAS1_AUDIT, GPS_LATITUD, GPS_LONGITUD, PORCION_ORIGINAL, SEC_ORIGINAL, UNIDAD_ORIGINAL, CODPOST, DCSM, DISTRITO, CIRC, SECCION, MANZANA, ORDENADO_POR, SEC_RELACIONADA, LAT, LON FROM qorder_suministros_gu where COD_UNICOM={} and ANIO={}'.format(
			oficina,strsemana)
...
def parceararchivo(path_txt, lista1, oficina, user, _log,flag_imp,strsemana):
	try:
		DataList = []
		lineas = []
		DataFileIn = []
		...
		with connection.cursor() as cursor:
			...
			for a in lista1:
				...
				try:
					...
					for item in DataList:  # Iterar sobre las filas - cada elemento es la cadena de datos
						if item.startswith('//'):  # Si la linea inicia con el caracter // saltearlo.
							continue

						if item.startswith('FFFF'):  # Si la linea termina con FFFF  finalizar.
							break
						else:

							if len(item) < 427:  # Si el largo de la linea es menor entonces la agrego a la lista para luego avisar que suministro es el que esta mal parceado.
								lista.append(item)
							else:
								if str(item[45:55]) == '          ':  #Campo de la linea vacio salteo la linea y la agrego como item con error
									_log.error('Registros incompletos {} del archivo {} '.format(str(item[0:18]), str(a)))
									continue
								else:
									#Buscamos el caracter y lo remplazamos ya que se utiliza para generar los archivos bulk insert y corre las lineas.
									item=item.replace("#","n")
									lineas.append(importador(item))
					...
					if listporcionesgu.count(files) > 0:
						# print('entrasumgu')
						# print(files)
						filesuministrogu(lineas, oficina, cursor, _log)

					else:
						# print('entrasumres')
						# print(files)
						filesuministrores(lineas, oficina, cursor, _log)

					_log.info('Se Importaron {} registros del archivo {} '.format(len(lineas), str(a)))

				except Exception as e:
					_log.error('Error al procesar el archivo {} '.format(str(a)))
					lista_registros_error.append(a)
			...
			#
			if flag_imp==1:
				#Obtengo los suministros importados de suministros_res y suministros_gu para no mezclarlos con los ya importados
				sumin_res=suministros_res.objects.filter(ANIO=strsemana,COD_UNICOM=oficina).values('SEG_REG')
				for sumin in sumin_res:
					lista_sumin.append(sumin['SEG_REG'])
				sumin_gu= suministros_gu.objects.filter(ANIO=strsemana, COD_UNICOM=oficina).values('SEG_REG')
				for sumin in sumin_gu:
					lista_sumin.append(sumin['SEG_REG'])
				strsuministros = ', '.join(lista_sumin)
				insertarsuministros(strsemana,oficina, cursor)  # Los inserto directamente en la tabla suministros sin reorganizarlos
			else:
				#Si se realiza por primera vez importacion realizo los updates necesarios para insertar luego en la tabla suministros
				updatetables(listporcionprocesos, _log, oficina, strsemana, cursor)

			#Obtngo los codigos , en caso de haber uno nuevo lo inserto en la tabla codigo
			...
			_log.info("Iniciando insert de clientes")
			insertCliente(_log, oficina,flag_imp,strsuministros,strsemana, cursor)

			_log.info("Iniciando insert de aparatos de suministros")
			insertAparatosSuministro(_log, oficina,flag_imp,strsuministros,strsemana, cursor)

			_log.info("Iniciando insert de aparatos de consumos")
			insertAparatoConsumo(_log, oficina,flag_imp,strsuministros,strsemana, cursor)

			_log.info("Iniciando insert de rutas")
			insertrutasum(_log, oficina,flag_imp,strsuministros,strsemana, cursor)

			_log.info("Iniciando insert de datos de suministros")
			insertDatosSuministro(_log, oficina,flag_imp,strsuministros,strsemana, cursor)

			_log.info("Iniciando insert de datos de rutas")
			insertruta(_log, oficina, user,flag_imp,strsuministros,strsemana, cursor)

			_log.info("Iniciando insert de órdenes")
			insertorden(_log, oficina,flag_imp,strsuministros,strsemana, cursor)
			
			_log.info("Iniciando insert de historicos de consumos")
			inserthistconsumo(_log, oficina,flag_imp,strsuministros,strsemana, cursor)

			_log.info('******************************************')
			
			if len(lista) == 0 and len(lista_registros_error) == 0:
				transaction.commit()
				_log.info('Cantidad total de suministros importados: {}'.format(total_lineas))
				_log.info('Se realizó Correctamente la Importación')
			else:
					
				if len(lista_registros_error) > 0:
					_log.info('---Revisar los logs de los archivos no insertados---')
					for error in lista_registros_error:
						_log.info('Archivo incorrecto: {}'.format(error))
				...
				_log.info('IMPORTACIÓN FALLIDA, SE REVIRTIÓ EL PROCESO EN SU TOTALIDAD - CONTACTARSE CON SERVICIO POSTVENTA')
